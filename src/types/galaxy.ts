...
export interface StarSystem {
  ...
  type?: 'background' | 'main'; // Type for rendering prioritization
  resources?: {
...
export interface StarSystemRenderInfo {
  ...
  parallaxOffset: Point2D; // Parallax offset for background stars
}
...
export interface GalacticSector {
  ...
  entropyLevel?: number; // 0-1, explicit entropy level for rendering
...
export interface SectorRenderInfo {
  ...
  lodLevel: number; // Level of detail (0-3)
  showLabels: boolean; // Whether to show sector labels
  showDetails: boolean; // Whether to show detailed sector info
}
