...
export class GameController {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn(
          '[GameController] Battery optimization failed to initialize:',
          error
        );
      }
  ...
  updateAutoSaveInterval(intervalSeconds: number): void {
    console.log(
      `[GameController] Updating auto-save interval to ${intervalSeconds}s`
    );
  ...
  setOfflineGenerationEnabled(enabled: boolean): void {
    console.log(
      `[GameController] ${enabled ? 'Enabling' : 'Disabling'} offline generation`
    );
  ...
  private notifyGameStateChanged(): void {
    this.gameStateChangeCallbacks.forEach(callback => {
      try {
        ...
        console.error(
          '[GameController] Error in game state change callback:',
          error
        );
      }
    });
  ...
  placeBeacon(
    ...
    const cost =
      this.resourceManager.calculateBeaconPlacementCost(currentBeaconCount);

    // Check if player has enough resources
    if (!this.resourceManager.canAfford(cost)) {
      return {
        success: false,
        error: `Insufficient resources: need ${cost.quantumData} Quantum Data`,
      };
    }
    ...
    if (result.success && result.beacon) {
      ...
      console.log(
        `[GameController] Placed ${type} beacon at (${position.x}, ${position.y})`
      );
    }
  ...
  placeBeaconWithFallback(
    ...
    if (result.success && result.beacon) {
      ...
      if (wasRelocated) {
        console.log(
          `[GameController] Placed ${type} beacon at fallback position (${finalPos.x}, ${finalPos.y}) instead of target (${targetPosition.x}, ${targetPosition.y})`
        );
      } else {
        console.log(
          `[GameController] Placed ${type} beacon at target position (${finalPos.x}, ${finalPos.y})`
        );
      }
    }
  ...
  getBeaconPlacementCost(
    ...
    const currentBeaconCount = Object.keys(this.gameState.beacons).length;
    return this.resourceManager.calculateBeaconPlacementCost(
      currentBeaconCount,
      specialization
    );
  }
  ...
  canAffordBeaconPlacement(
    ...
    const currentBeaconCount = Object.keys(this.gameState.beacons).length;
    return this.resourceManager.canAffordBeaconPlacement(
      currentBeaconCount,
      specialization
    );
  }
  ...
  private updateBeaconConnections(): void {
    ...
    const connectionCount =
      this.beaconConnectionManager.getAllConnections().length;
    console.log(
      `[GameController] Updated beacon connections: ${connectionCount} active connections`
    );
  }
  ...
  private loadBeaconsIntoManager(): void {
    ...
    console.log(
      `[GameController] Loaded ${Object.keys(this.gameState.beacons).length} beacons into placement manager`
    );
  ...
  private loadUpgradeData(): void {
    ...
    if (this.gameState.upgrades) {
      ...
      console.log(
        '[GameController] No upgrade data found, starting with fresh upgrade state'
      );
    }
  ...
  private startAutoSave(): void {
    ...
    console.log(
      `[GameController] Auto-save started (interval: ${this.config.autoSaveInterval}s)`
    );
  }
  ...
  private async handleAppStateChange(
    ...
    if (nextAppState === 'background' || nextAppState === 'inactive') {
      ...
      try {
        ...
        console.error(
          '[GameController] Failed to save before background:',
          error
        );
      }
      ...
      try {
        ...
        console.error(
          '[GameController] Failed to handle offline progression:',
          error
        );
      }
    }
  ...
  private async handleOfflineProgression(): Promise<void> {
    ...
    if (cappedOfflineTime > 60000) {
      ...
      console.log(
        `[GameController] Processing ${offlineHours.toFixed(2)} hours of offline progression`
      );
      ...
      if (this.config.enableBackgroundProcessing) {
        ...
        console.log(
          '[GameController] Offline resource generation disabled, skipping'
        );
      }
  ...
  private async processOfflineResourceGeneration(
    ...
    console.log(
      `[GameController] Processing ${totalChunks} chunks of offline progression`
    );

    for (let chunk = 0; chunk < totalChunks; chunk++) {
      const chunkDuration = Math.min(
        chunkSize,
        offlineSeconds - chunk * chunkSize
      );
    ...
    if (totalGenerated.length > 0) {
      console.log(
        `[GameController] Generated offline: ${totalGenerated.join(', ')}`
      );
    }
  }
