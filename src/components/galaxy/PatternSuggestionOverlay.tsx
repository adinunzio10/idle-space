...
const PatternSuggestionOverlayPropsAreEqual = (
  ...
  if (
    ...
    prevSuggestionState.mapVisualizationsVisible !==
      nextSuggestionState.mapVisualizationsVisible ||
    prevSuggestionState.displayMode !== nextSuggestionState.displayMode ||
    prevSuggestionState.selectedSuggestion?.id !==
      nextSuggestionState.selectedSuggestion?.id ||
    prevSuggestionState.hoveredSuggestion?.id !==
      nextSuggestionState.hoveredSuggestion?.id
  ) {
    return false;
  }
...
export const PatternSuggestionOverlay: React.FC<PatternSuggestionOverlayProps> =
  memo(
    ({
      suggestions,
      beacons,
      ...
      const visibleSuggestions = useMemo(() => {
        ...
        let filtered = suggestions.filter(
          s => !suggestionState.dismissedSuggestions.has(s.id)
        );
        ...
        switch (suggestionState.displayMode) {
          ...
          default:
            filtered = filtered.slice(
              0,
              DEFAULT_PLACEMENT_HINT_CONFIG.maxHints
            );
            break;
        }
        ...
        suggestionState.dismissedSuggestions,
        suggestionState.displayMode,
      ]);
        ...
        <G>
          ...
          {visibleSuggestions.map(suggestion => (
            <SuggestionVisualization
              ...
              viewportState={viewportState}
              isSelected={
                suggestionState.selectedSuggestion?.id === suggestion.id
              }
              isHovered={
                suggestionState.hoveredSuggestion?.id === suggestion.id
              }
...
const SuggestionVisualization: React.FC<SuggestionVisualizationProps> = memo(
  ({
    ...
    const requiredBeaconScreenPositions = useMemo(() => {
      return suggestion.requiredBeacons
        .map(beaconId => {
    ...
    const animatedGhostProps = useAnimatedProps(() => {
      const scale = interpolate(appearProgress.value, [0, 1], [0.5, 1]);
...
const BonusIndicator: React.FC<BonusIndicatorProps> = memo(
  ({ position, suggestion, isVisible, enableAnimations }) => {
    ...
    return (
      ...
      >
        ...
        <SvgText x="0" y="8" fontSize="8" textAnchor="middle" fill="#9CA3AF">
          {completionText}
        </SvgText>
      </AnimatedG>
    );
  ...
  const actions = useMemo(
    () => ({
      ...
      showMapVisualizations: () =>
        setState(s => ({ ...s, mapVisualizationsVisible: true })),
      hideMapVisualizations: () =>
        setState(s => ({ ...s, mapVisualizationsVisible: false })),
