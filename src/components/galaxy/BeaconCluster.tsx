...
import { Circle, Text, G, Defs, RadialGradient, Stop } from 'react-native-svg';
...
export const BeaconClusterRenderer: React.FC<BeaconClusterRendererProps> = memo(
  ({ cluster, viewportState, onPress }) => {
    ...
    const typeCount = cluster.beacons.reduce(
      (counts, beacon) => {
    ...
    const radius = useMemo(() => {
      ...
      return Math.max(
        baseRadius,
        Math.min(baseRadius * 2, baseRadius + scaleFactor * 10)
      );
    ...
    const compositionRings = useMemo(() => {
      ...
      const types = Object.keys(
        typeCount
      ) as (keyof typeof RENDERING_CONFIG.BEACON_COLORS)[];
