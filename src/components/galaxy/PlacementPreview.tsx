...
export const PlacementPreview: React.FC<PlacementPreviewProps> = ({
  ...
  const connectionPreviews = useMemo(() => {
    ...
    return placementInfo.estimatedConnections.map(beaconId => {
  ...
  }, [placementInfo.type, screenPosition, viewportState.scale, validationColor, opacity]);
  ...
  const validationFeedback = useMemo(() => {
    ...
    return placementInfo.validationReasons.slice(0, maxReasons).map((reason, index) => (
      <text
  ...
  }, [placementInfo.isValid, placementInfo.validationReasons, screenPosition, opacity]);
