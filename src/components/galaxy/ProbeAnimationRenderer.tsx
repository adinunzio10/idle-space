...
const AnimatedProbe: React.FC<AnimatedProbeProps> = React.memo(
  ({ probe, scale, translateX, translateY, screenWidth, screenHeight }) => {
  ...
  const activeProbes = probes.filter(
    probe =>
      ...
      (probe.status === 'deployed' &&
        Date.now() - (probe.deploymentCompletedAt || 0) < 5000) // Show for 5 seconds after deployment (matches ProbeManager)
  );
