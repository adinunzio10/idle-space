...
export const PatternRenderer: React.FC<PatternRendererProps> = memo(({
  ...
  const lodLevel = useMemo(() => {
    ...
    if (scale >= PATTERN_VISUAL_CONFIG.LOD_SETTINGS.FULL_DETAIL.minZoom) {
      ...
      return 3; // Standard
    } else if (scale >= PATTERN_VISUAL_CONFIG.LOD_SETTINGS.SIMPLIFIED.minZoom) {
      return 2; // Simplified
    } else {
      return 1; // Hidden
    }
  ...
  const lodSettings = useMemo(() => {
    switch (lodLevel) {
      case 4: return PATTERN_VISUAL_CONFIG.LOD_SETTINGS.FULL_DETAIL;
      case 3: return PATTERN_VISUAL_CONFIG.LOD_SETTINGS.STANDARD;
      case 2: return PATTERN_VISUAL_CONFIG.LOD_SETTINGS.SIMPLIFIED;
      default: return PATTERN_VISUAL_CONFIG.LOD_SETTINGS.HIDDEN;
    }
  }, [lodLevel]);
  ...
  const visiblePatterns = useMemo(() => {
    ...
    const patternsInView = patterns.filter(pattern => {
      ...
      return patternBeacons.some(beacon => {
        return beacon.position.x >= visibleBounds.minX - margin &&
  ...
  }, [patterns, beacons, lodLevel, qualitySettings.maxPatterns, visibleBounds]);
  ...
  const patternGroups = useMemo(() => {
    ...
    visiblePatterns.forEach(pattern => {
      ...
      groups.push({
        patterns: groupPatterns.sort((a, b) => 
          PATTERN_OVERLAP.Z_INDEX_PRIORITY[b.type] - PATTERN_OVERLAP.Z_INDEX_PRIORITY[a.type]
        ),
        zIndex: maxZIndex,
      });
    });
  ...
  const getPatternBonusInfo = useCallback((pattern: GeometricPattern) => {
  ...
  const handlePatternPress = useCallback((pattern: GeometricPattern) => {
          ...
          {group.patterns.map((pattern, patternIndex) => {
            ...
            return (
              <PatternOverlay
                ...
                lodLevel={lodLevel}
                enableAnimations={qualitySettings.enableAnimations && lodSettings.enableAnimations}
              />
            );
          })}
        </G>
