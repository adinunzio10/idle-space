...
export const PatternOverlay: React.FC<PatternOverlayProps> = memo(({
  ...
  const animatedPolygonProps = useAnimatedProps(() => {
    ...
    const opacity = interpolate(
      formationProgress.value,
      [0, 0.3, 1],
      [0, 0.5, isActive ? PATTERN_VISUAL_CONFIG.RENDERING.FILL_OPACITY_ACTIVE : PATTERN_VISUAL_CONFIG.RENDERING.FILL_OPACITY]
    );
  ...
  return (
    <AnimatedG onPress={onPress ? () => onPress(pattern) : undefined}>
      <Defs>
        ...
        {showGlow && (
          <LinearGradient
            ...
            y2="100%"
          >
            <Stop offset="0%" stopColor={colors.glowActive} stopOpacity={0.8} />
            <Stop offset="50%" stopColor={colors.glow} stopOpacity={0.4} />
            <Stop offset="100%" stopColor={colors.glowActive} stopOpacity={0.8} />
          </LinearGradient>
        )}
        ...
        {showShimmer && (
          ...
          >
            ...
            <Stop offset="20%" stopColor={colors.glowActive} stopOpacity={0.3} />
            <Stop offset="50%" stopColor={colors.glowActive} stopOpacity={0.8} />
            <Stop offset="80%" stopColor={colors.glowActive} stopOpacity={0.3} />
      ...
      <AnimatedPolygon
        ...
        strokeWidth={isActive ? PATTERN_VISUAL_CONFIG.RENDERING.STROKE_WIDTH_ACTIVE : PATTERN_VISUAL_CONFIG.RENDERING.STROKE_WIDTH}
        strokeOpacity={isActive ? PATTERN_VISUAL_CONFIG.RENDERING.STROKE_OPACITY_ACTIVE : PATTERN_VISUAL_CONFIG.RENDERING.STROKE_OPACITY}
      />
...
const PatternParticles: React.FC<PatternParticlesProps> = memo(({
  ...
  const particles = Array.from({ length: patternType === 'hexagon' ? 8 : 6 }, (_, i) => {
