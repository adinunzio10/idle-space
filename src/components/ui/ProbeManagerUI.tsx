...
export const ProbeManagerUI: React.FC<ProbeManagerUIProps> = ({
  ...
  useEffect(() => {
    ...
    const removeProbeUpdateCallback =
      probeManager.addProbeUpdateCallback(updateProbeStatus);
  ...
  const launchDemoProbe = async (type: ProbeType) => {
    try {
      ...
      if (result.success) {
        ...
        console.warn(
          `[ProbeManagerUI] Failed to launch ${type} probe:`,
          result.error
        );
      }
    } catch (error) {
    ...
    <View className="flex-1 bg-surface rounded-t-xl overflow-hidden">
      ...
      <View className="bg-surface px-4 py-3">
        ...
        <Text className="text-text/60 text-sm mt-1">
          {queueStatus.totalProbes} probes • {queueStatus.queuedProbes.length}{' '}
          queued • {queueStatus.activeProbes.length} active
        </Text>
      </View>
      ...
      <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-6">
          ...
          <Text className="text-accent text-sm mb-3">
            ⚡ Manual launches get automatic 2x speed boost
          </Text>
        ...
        {queueStatus.totalProbes === 0 && (
          <View className="items-center justify-center py-8">
            <Text className="text-text/40 text-base">
