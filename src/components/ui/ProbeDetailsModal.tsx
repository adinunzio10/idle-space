...
interface ProbeDetailsModalProps {
  isVisible: boolean;
  probe?: ProbeInstance;
  onClose: () => void;
  gameController?: any;
...
export const ProbeDetailsModal: React.FC<ProbeDetailsModalProps> = ({
  isVisible,
  probe,
  onClose,
  gameController
  ...
  const probeStats = useMemo(() => {
    ...
    const totalTravelTime = probe.deploymentStartedAt ? (probe.deploymentStartedAt - probe.createdAt) : 30000; // Default 30s
    ...
    switch (type) {
      ...
      case 'harvester':
        return {
          ...
          specialties: ['Resource detection', 'Efficient deployment', 'Area analysis']
        };
      case 'architect':
        return {
          emoji: '🏗️',
          ...
          color: 'text-accent',
          specialties: ['Pattern analysis', 'Precise positioning', 'Network optimization']
        };
      default:
        return {
          emoji: '🛸',
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            Alert.alert('Probe Recalled', 'The probe has been recalled and is returning.');
            onClose();
          }
        }
      ]
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            Alert.alert('Probe Boosted', 'The probe has arrived at its destination!');
            onClose();
          }
        }
      ]
    ...
    >
      <View className="space-y-6">
        ...
        <View className="bg-background/50 rounded-lg p-4">
          <Text className="text-text font-semibold text-lg mb-3">Mission Status</Text>
          
          <View className="items-center mb-4">
            <View className={`px-3 py-2 rounded-full mb-2`} style={{ backgroundColor: statusInfo.color.replace('text-', '') + '20' }}>
          ...
          {!probeStats.isArrived && (
            <View className="mb-4">
              <View className="flex-row justify-between mb-2">
                ...
                <Text className="text-text font-semibold">{probeStats.progress.toFixed(1)}%</Text>
              </View>
        ...
        {/* Mission Details */}
        <View className="bg-background/50 rounded-lg p-4">
          <Text className="text-text font-semibold text-lg mb-3">Mission Details</Text>
          
          <View className="space-y-2">
            <View className="flex-row justify-between">
              <Text className="text-text/70">Target Position</Text>
              <Text className="text-text font-semibold">{probeStats.targetPosition}</Text>
            </View>
            
            <View className="flex-row justify-between">
              <Text className="text-text/70">Mission Time</Text>
              <Text className="text-text font-semibold">{formatTime(probeStats.deployedTime)}</Text>
            </View>
            
            <View className="flex-row justify-between">
              <Text className="text-text/70">Fuel Used</Text>
              <Text className="text-text font-semibold">{probeStats.fuelUsed}%</Text>
            </View>
        ...
        {/* Probe Capabilities */}
        <View className="bg-background/50 rounded-lg p-4">
          <Text className="text-text font-semibold text-lg mb-3">Capabilities</Text>
