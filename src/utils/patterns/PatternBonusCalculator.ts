...
export class PatternBonusCalculator {
   ...
   */
  private detectOverlappingPatterns(patterns: GeometricPattern[]): PatternOverlap[] {
  ...
  private analyzePatternOverlap(
    ...
    const overlapType = this.classifyOverlapType(pattern1, pattern2, sharedBeacons);
    const overlapSeverity = this.calculateOverlapSeverity(pattern1, pattern2, sharedBeacons);
  ...
  private hasSharedEdge(
    ...
    for (let i = 0; i < sharedBeacons.length; i++) {
      for (let j = i + 1; j < sharedBeacons.length; j++) {
        ...
        if (this.areConsecutiveInPattern(beacon1, beacon2, pattern1) &&
  ...
  private calculateSingleBeaconContribution(
    ...
    const connectionQualityMultiplier = this.calculateConnectionQualityBonus(beacon);
    ...
    switch (this.config.strategy) {
      case 'multiplicative':
        effectiveMultiplier = multipliers.reduce((acc, mult) => acc * mult, 1 as number);
        break;
      case 'additive':
        effectiveMultiplier = 1 + multipliers.reduce((acc, mult) => acc + (mult - 1), 0 as number);
        break;
      case 'maximum':
  ...
  private calculateConnectionQualityBonus(beacon: Beacon): number {
    ...
    const qualityLevel = Math.min(5, Math.max(1, Math.floor((beaconLevel + connectionCount) / 2)));
    
    return CONNECTION_QUALITY_BONUSES[qualityLevel as keyof typeof CONNECTION_QUALITY_BONUSES] || 1;
  }
  ...
  private applyOverlapModifiers(
    ...
    for (const overlap of relevantOverlaps) {
      ...
      const weightedModifier = 1 + (overlapModifier - 1) * overlap.overlapSeverity;
   ...
   * @returns Total multiplier
   */
  private applyMultiplicativeStacking(contributions: BeaconBonusContribution[]): number {
    ...
    for (const contrib of contributions) {
      ...
      const contributionPower = Math.pow(contrib.effectiveMultiplier, weightRatio);
   ...
   * @returns Total multiplier
   */
  private applyAdditiveStacking(contributions: BeaconBonusContribution[]): number {
    const totalWeight = contributions.reduce((sum, contrib) => sum + contrib.weight, 0);
   ...
   * @returns Total multiplier
   */
  private applyMaximumStacking(contributions: BeaconBonusContribution[]): number {
    if (contributions.length === 0) return 1;
   ...
   * @returns Total multiplier
   */
  private applyWeightedStacking(contributions: BeaconBonusContribution[]): number {
  ...
  private generatePatternBreakdown(
    ...
    for (const pattern of patterns) {
      ...
      const effectiveBonus = patternContributions.length > 0
        ? patternContributions.reduce((sum, c) => sum + c.effectiveMultiplier, 0) / patternContributions.length
        : baseBonus;
   ...
   */
  private calculateBaseGeneration(beacons: Beacon[]): Partial<Record<ResourceType, number>> {
  ...
  private applyBonusesToGeneration(
    ...
    for (const [resourceType, baseRate] of Object.entries(baseGeneration)) {
      if (baseRate && this.config.targetResourceTypes.includes(resourceType as ResourceType)) {
        const resourceWeight = RESOURCE_BONUS_WEIGHTS[resourceType as ResourceType] || 1;
        const effectiveMultiplier = 1 + (totalMultiplier - 1) * resourceWeight;
        bonusedGeneration[resourceType as ResourceType] = baseRate * effectiveMultiplier;
      }
    }
   ...
   * @returns Cache key
   */
  private generateCacheKey(patterns: GeometricPattern[], beacons: Beacon[]): string {
  ...
  validateResult(result: BonusCalculationResult): BonusValidationResult {
    ...
    if (result.beaconContributions.length === 0 && result.totalMultiplier !== 1) {
