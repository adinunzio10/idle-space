...
function calculateConnectionStrength(
  ...
  if (
    (sourceBeacon.type === 'harvester' && targetBeacon.type === 'pioneer') ||
...
export class PatternDetector {
  ...
  constructor(
    ...
    this.suggestionEngine = new PatternSuggestionEngine(
      this.spatialHash,
      DEFAULT_TOLERANCE,
      placementValidator,
      spatialIndex
  ...
  detectPatternsOptimized(
    ...
    if (
      beaconHash === this.lastBeaconHash &&
      this.cachedPatterns.has(beaconHash)
    ) {
    ...
    if (
      beacons.length > SPATIAL_PERFORMANCE_THRESHOLDS.HIGH_DENSITY_THRESHOLD
    ) {
      ...
      patterns = this.detectPatternsWithTriangulation(
        beacons,
        connections,
        beaconHash
      );
    ...
    if (this.optimizationSettings.debugMode) {
      console.log(
        `Pattern detection took ${detectionTime.toFixed(2)}ms for ${beacons.length} beacons (${patterns.length} patterns found)`
      );
    }
   ...
   */
  private detectPatternsWithSpatialHashing(
    beacons: Beacon[],
    connections: Connection[]
  ): GeometricPattern[] {
    const patterns: GeometricPattern[] = [];
    ...
    for (const beacon of beacons) {
      ...
      const localPatterns = this.findPatternsFromNeighbors(
        beacon,
        neighbors
          .map(n => beacons.find(b => b.id === n.beacon.id)!)
          .filter(Boolean),
        beacons,
      ...
      if (
        patterns.length >
        SPATIAL_PERFORMANCE_THRESHOLDS.CACHE_AGGRESSIVE_THRESHOLD
      ) {
        break;
      }
    }
        ...
        for (let j = i + 1; j < neighborsArray.length; j++) {
          ...
          if (neighborMap.get(neighbor1)?.has(neighbor2)) {
            ...
            if (!processedTriangles.has(triangleKey)) {
              ...
              if (
                defaultShapeDetector.detectTriangle(
                  Array.from(beaconMap.values()),
                  triangle
                )
              ) {
          ...
          for (const neighbor3 of neighbor2Neighbors) {
            ...
            if (neighborMap.get(neighbor3)?.has(beaconId)) {
              ...
              if (!processedSquares.has(squareKey)) {
                ...
                if (
                  defaultShapeDetector.detectSquare(
                    Array.from(beaconMap.values()),
                    square
                  )
                ) {
   ...
   */
  getCachedBonusResult(
    beaconHash?: string
  ): BonusCalculationResult | undefined {
    const hash = beaconHash || this.lastBeaconHash;
