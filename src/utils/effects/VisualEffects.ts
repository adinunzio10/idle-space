...
export function createProbeLaunchAnimation(
  ...
  // Scale punch effect
  scaleValue.value = withSequence(
    withTiming(0.8, { duration: config.duration * 0.3, easing: VISUAL_EASINGS.cubic }),
    withTiming(1.2, { duration: config.duration * 0.7, easing: config.easing }),
    withTiming(1.0, { duration: config.duration * 0.3, easing: VISUAL_EASINGS.ease })
  );

  // Glow burst
  glowValue.value = withSequence(
    withTiming(2.0, { duration: config.duration * 0.5, easing: config.easing }),
    withTiming(0, { duration: config.duration * 0.5, easing: VISUAL_EASINGS.ease })
  );
}
...
export const COLOR_EFFECTS = {
  ...
  createPulsingColor: (baseColor: string, accentColor: string, progress: number): string => {
    const pulseIntensity = Math.abs(Math.sin(progress * Math.PI));
    return COLOR_EFFECTS.interpolateColor(pulseIntensity, baseColor, accentColor);
  },
  ...
  createEnergyColor: (progress: number): string => {
    ...
    return COLOR_EFFECTS.interpolateColor(t, colors[startIndex], colors[endIndex]);
  },
};
...
export const PHYSICS_EFFECTS = {
  ...
  createGravityEffect: (
    ...
    const distance = initialVelocity * (duration / 1000) + 0.5 * gravity * Math.pow(duration / 1000, 2);
