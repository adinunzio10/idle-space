...
export function getConnectionRenderInfo(
  ...
  const strengthMapping =
    CONNECTION_CONFIG.STRENGTH_MAPPING[
      connection.strength as keyof typeof CONNECTION_CONFIG.STRENGTH_MAPPING
    ] || CONNECTION_CONFIG.STRENGTH_MAPPING[1];
  const baseWidth = strengthMapping.width;
  const lineWidth = Math.max(
    CONNECTION_CONFIG.VISUAL.MIN_WIDTH,
    Math.min(CONNECTION_CONFIG.VISUAL.MAX_WIDTH, baseWidth)
  );

  // Calculate opacity
  let opacity: number = connection.isActive
    ? CONNECTION_CONFIG.VISUAL.ACTIVE_OPACITY
    : CONNECTION_CONFIG.VISUAL.BASE_OPACITY;
...
export function calculateConnectionCurve(
  ...
  const direction = connectionId
    ? connectionId
      ...
      ? 1
      : -1
    : Math.sin(source.x + target.x + source.y + target.y) > 0
      ? 1
      : -1;
...
export function generateConnectionPath(
  ...
  const curve = calculateConnectionCurve(
    source,
    target,
    strength,
    connectionId
...
export function prioritizeConnections(
  ...
  return connections.sort((a, b) => {
    ...
    if (a.connection.patterns.length > 0 && b.connection.patterns.length === 0)
      return -1;
    if (a.connection.patterns.length === 0 && b.connection.patterns.length > 0)
      return 1;
    ...
    const distanceA = calculateConnectionDistance(
      a.connection,
      a.source,
      a.target
    );
    const distanceB = calculateConnectionDistance(
      b.connection,
      b.source,
      b.target
    );
