...
export class BatteryOptimizationManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      try {
        this.batteryUpdateSubscription = Battery.addBatteryLevelListener(
          this.handleBatteryLevelChange
        );
      } catch (error) {
        console.warn(
          '[BatteryOptimization] Battery listener not available on this platform:',
          error
        );
      }
  ...
  private handleAppStateChange = (nextAppState: AppStateStatus) => {
    if (nextAppState === 'background' && this.config.backgroundThrottling) {
      ...
      this.state.backgroundProcessingScale = Math.min(
        0.5,
        this.state.backgroundProcessingScale
      );
  ...
  private assessOptimizationNeeds(): void {
    ...
    const previousBackgroundProcessingScale =
      this.state.backgroundProcessingScale;

    // Determine optimization level
    if (
      this.config.batteryEfficientMode ||
    ...
    this.state.isBatteryOptimized =
      this.state.currentOptimizationLevel !== 'none';

    // Notify callbacks if significant changes occurred
    if (
      previousOptimizationLevel !== this.state.currentOptimizationLevel ||
      previousTargetFrameRate !== this.state.targetFrameRate ||
      previousVisualEffectsEnabled !== this.state.visualEffectsEnabled ||
      Math.abs(
        previousBackgroundProcessingScale - this.state.backgroundProcessingScale
      ) > 0.1
    ) {
      this.notifyCallbacks();
    }
  ...
  private adaptFrameRateBasedOnPerformance(currentFps: number): void {
    ...
    if (
      avgFps < this.state.targetFrameRate - 10 &&
      this.state.batteryLevel < 0.8
    ) {
    ...
    // If consistently above target and battery is good, can increase target
      ...
      avgFps > this.state.targetFrameRate + 5 &&
      this.state.batteryLevel > 0.6
    ) {
  ...
  private getAverageFPS(): number {
    ...
    const avgFps =
      recentEntries.reduce((sum, entry) => sum + entry.fps, 0) /
  ...
  private estimateTimeRemaining(): number | undefined {
    ...
    const batteryDrop =
      recentEntries[0].batteryLevel -
      recentEntries[recentEntries.length - 1].batteryLevel;
...
export function useBatteryOptimization() {
  const [state, setState] = React.useState(() =>
    batteryOptimizationManager.getState()
  );
  const [metrics, setMetrics] = React.useState(() =>
    batteryOptimizationManager.getMetrics()
  );
