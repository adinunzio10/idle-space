...
export class AccessibilityHelper {
   ...
   */
  static initializeListeners(callbacks: {
   ...
   */
  static announceForAccessibility(message: string, options: { queue?: boolean } = {}): void {
  ...
  static getAccessibilityProps(config: {
    ...
    // State properties
    if (state) {
      if (state.disabled !== undefined) props.accessibilityState = { ...props.accessibilityState, disabled: state.disabled };
      if (state.selected !== undefined) props.accessibilityState = { ...props.accessibilityState, selected: state.selected };
      if (state.checked !== undefined) props.accessibilityState = { ...props.accessibilityState, checked: state.checked };
      if (state.expanded !== undefined) props.accessibilityState = { ...props.accessibilityState, expanded: state.expanded };
      if (state.busy !== undefined) props.accessibilityState = { ...props.accessibilityState, busy: state.busy };
    }
   ...
   */
  static describeGameElement(type: 'beacon' | 'probe' | 'resource' | 'button', details: any): string {
    switch (type) {
      case 'beacon':
      ...
      case 'button':
        ...
        if (available !== undefined) buttonDesc += available ? ', available' : ', unavailable';
        return buttonDesc;
  ...
  static describeUIState(state: {
    ...
    if (progress) {
      parts.push(`${progress.task}: ${progress.current} of ${progress.total} complete`);
    }
  ...
  static validateAccessibility(element: {
    ...
    if (element.isInteractive) {
      if (!element.hasLabel) issues.push('Interactive element missing accessibility label');
      if (!element.hasRole) issues.push('Interactive element missing accessibility role');
      if (!element.hasFocusState) issues.push('Interactive element missing focus state');
    }
    
    if (!element.hasContrast) issues.push('Element may not meet contrast requirements');
