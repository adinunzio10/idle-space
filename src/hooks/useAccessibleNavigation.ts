...
export const useAccessibleNavigation = () => {
  ...
  const navigateWithAccessibility = useCallback(
    ...
    [
      navigation,
      config.accessibilityMode.reduceMotionEnabled,
      announceNavigation,
    ]
  );

  const goBackWithAccessibility = useCallback(
    (options?: { haptic?: boolean; announcement?: string }) => {
      const { haptic = true, announcement = 'Went back to previous screen' } =
        options || {};
  ...
  const createAccessibleTabBar = useCallback(
    (
      tabs: {
        key: string;
        title: string;
  ...
  const navigationShortcuts = {
    ...
    toStatistics: () =>
      navigateWithAccessibility('Statistics', undefined, {
        announcement: 'Opened statistics - view your game progress',
      }),
    toPatterns: () =>
      navigateWithAccessibility('PatternGallery', undefined, {
        announcement: 'Opened pattern gallery - browse discovered formations',
      }),

    // Contextual navigation with parameters - these would use overlays instead of navigation
    // in the current architecture since BeaconDetails and ProbeDetails are overlays
    toProbeManager: () =>
      navigateWithAccessibility('ProbeManager', undefined, {
