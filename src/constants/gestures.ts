...
export class GestureConfiguration {
  ...
  public getPanThresholds() {
    ...
    return {
      minDistance: base.MIN_DISTANCE * profile.panSensitivity * 
        (this.accessibilitySettings.motorImpairment ? ACCESSIBILITY.MOTOR.REDUCED_PRECISION : 1),
  ...
  public getTapThresholds() {
    ...
    return {
      maxDuration: this.accessibilitySettings.motorImpairment ? 
        ACCESSIBILITY.MOTOR.LONGER_TAP_DURATION : base.MAX_DURATION,
  ...
  public getHitRadius(baseRadius: number, scale: number): number {
    ...
    const profileAdjusted = scaleAdjustedRadius * this.currentProfile.adjustments.tapTolerance;
    
    return profileAdjusted * (
      this.accessibilitySettings.motorImpairment ? 
        ACCESSIBILITY.MOTOR.INCREASED_HIT_RADIUS : 1
    );
  }
  ...
  public enableAccessibility(type: 'motor' | 'visual' | 'cognitive', enabled: boolean) {
    switch (type) {
      case 'motor':
...
export const GestureUtils = {
  ...
  isPalmTouch(touchData: {
    ...
    if (touchData.width && touchData.height) {
      const aspectRatio = Math.max(touchData.width, touchData.height) / 
  ...
  smoothVelocity(
    ...
    if (deltaX > config.smoothing.spikeThreshold || 
  ...
  applyMomentumDecay(velocity: { x: number; y: number }): { x: number; y: number } {
    const config = gestureConfig.getMomentumConfig();
    return {
